...
describe("2.1 Корректный сброс пароля. Почта: корректная", () => {
    it("passes", () => {
        ...
        cy.get("button").click();
        cy.get('[class="mb-4 text-sm font-medium text-green-600"]')
            .then(($el) => $el[0])
            .invoke("text")
            .should("contain", "We have emailed your password reset link.");
    });
});
...
describe("3.1 Не корректный сброс пароля. Почта: корректная. Слишком быстрый повторный сброс.", () => {
    it("passes", () => {
        ...
        cy.get("form")
            .then(($el) => $el[0].children[0])
            .last()
            .should("contain", "Please wait before retrying."); // Оба рабочих варианта
