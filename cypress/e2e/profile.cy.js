...
const $ = {
    ...
    buttonSaveInfo: "#info-submit",
    buttonSaveNewPass: "#password-submit",
...
// /** 3.1.1 Корректные изменения в профиле. Добавление, изменение, удаление аватара (позитивный сценарий) */

// describe("3.1.1.1 Проверка текста при добавлении аватара", () => {
//     it("passes", () => {
//         auth.login(Env.email.correct, Env.password.correct);
//         cy.visit(Env.url.domain + Env.url.profile);
...
// describe("3.1.1.2 Добавление аватара в профиль", () => {
//     it("passes", () => {
//         auth.login(Env.email.correct, Env.password.correct);
//         cy.visit(Env.url.domain + Env.url.profile);
//         cy.get($.imageForm).then((el) => {
...
// describe("3.1.1.3 Смена аватара в профиле", () => {
//     it("passes", () => {
//         let Src;
//         let newSrc;
...
// describe("3.1.1.4 Удаление аватара в профиле", () => {
//     it("passes", () => {
//         auth.login(Env.email.correct, Env.password.correct);
//         cy.visit(Env.url.domain + Env.url.profile);
...
// describe("3.1.1.5 Удаление аватаров из загруженных картинок", () => {
//     it("passes", () => {
//         auth.login(Env.email.correct, Env.password.correct);
...
/** 3.1.2 Корректные изменения в профиле. (Позитивный сценарий) */

describe("3.1.2.1 Обновление информацию о себе", () => {
    it("passes", () => {
        auth.login(Env.email.correct, Env.password.correct);
        cy.visit(Env.url.domain + Env.url.profile);
        cy.get($.nick).type(Env.name.newCorrect);
        cy.get($.bioEmail).type(Env.info.bioEmail);
        cy.get($.phone).type(Env.info.phone);
        cy.get($.job).type(Env.info.job);
        cy.get($.city).type(Env.info.city);
        cy.get($.link).type(Env.info.link);
        cy.get($.description).type(Env.info.description);
        cy.get($.buttonSaveBio).click();
        auth.logOut();

        auth.login(Env.email.correct, Env.password.correct);
        cy.visit(Env.url.domain + Env.url.profile);
        cy.get($.nick).then((el) => el[0].value).should("eq", Env.name.newCorrect);
        cy.get($.bioEmail).then((el) => el[0].value).should("eq", Env.info.bioEmail);
        cy.get($.phone).then((el) => el[0].value).should("eq", Env.info.phone);
        cy.get($.job).then((el) => el[0].value).should("eq", Env.info.job);
        cy.get($.city).then((el) => el[0].value).should("eq", Env.info.city);
        cy.get($.link).then((el) => el[0].value).should("eq", Env.info.link);
        cy.get($.description).then((el) => el[0].value).should("eq", Env.info.description);
    });
});

// describe("3.1.2.2 Обновление персональных данных", () => {
//     it("passes", () => {
//         auth.login(Env.email.correct, Env.password.correct);
//         cy.visit(Env.url.domain + Env.url.profile);
//         cy.get($.name).type(Env.name.newCorrect);
//         cy.get($.email).type(Env.email.newCorrect);
//         cy.get($.buttonSaveInfo).click();
//         cy.wait(1000);
//         auth.logOut();

//         auth.login(Env.email.newCorrect, Env.password.correct);
//         cy.get($.name).should("eq", Env.name.newCorrect);
//         cy.get($.email).should("eq", Env.email.newCorrect);
...
// describe("3.1.2.3 Изменить пароль", () => {
//     it("passes", () => {
//         auth.login(Env.email.correct, Env.password.correct);
//         cy.visit(Env.url.domain + Env.url.profile);
//         cy.get($.current_password).type(Env.password.correct);
//         cy.get($.password).type(Env.password.newCorrect);
//         cy.get($.password_confirmation).type(Env.password.newCorrect);
//         cy.get($.buttonSaveNewPass).click()

//         auth.logOut();
//         auth.login(Env.email.correct, Env.password.newCorrect);

//         cy.get($.current_password).type(Env.password.newCorrect);
